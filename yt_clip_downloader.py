...
def get_download_dir():
    """
    Return the best downloads folder for the current platform.
      - Termux (Android) : /sdcard/Download  (internal storage)
      - Windows          : C:\\Users\\<user>\\Downloads
      - macOS / Linux    : ~/Downloads  (fallback)
    """
    # Termux on Android exposes internal storage at /sdcard
    if os.path.isdir("/sdcard"):
        path = "/sdcard/Download"
    elif os.name == "nt":                          # Windows
        path = os.path.join(os.path.expanduser("~"), "Downloads")
    else:                                          # macOS / generic Linux
        path = os.path.join(os.path.expanduser("~"), "Downloads")
